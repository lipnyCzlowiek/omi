...
export class AppComponent {
	...
	obliczKlasyfikacjeDlaMasyKoluwium(
		...
		const isNumbers = value.match(this.patternNumbers) != null;
		const isLetters = value.match(this.patternLetters) !== null;

		if (!isNumbers || (isNumbers && isLetters) || isNaN(value)) {
			this.input.masaKoluwium = new String("N");
			this.output.m = 0;
			return;
		}
	...
	obliczKlasyfikacjeDlaObjetosciKoluwium(
		...
		if (!isNumbers || (isNumbers && isLetters) || isNaN(value)) {
			this.input.objetoscKoluwium = new String('N');
			this.output.v = 0;
			return;
		}
	...
	obliczKlasyfikacjeDlaPredkosciKoluwium(
		...
		if (!isNumbers || (isNumbers && isLetters) || isNaN(value)) {
			this.input.predkoscKoluwium = new String('N');
			this.output.s = 0;
			return;
		}
	...
	obliczKlasyfikacjeDlaLiczbyOfiar(value: any = this.input.liczbaOfiar): void {
		...
		if (!isNumbers || (isNumbers && isLetters) || isNaN(value)) {
			this.input.liczbaOfiar = new String('N');
			this.output.o = 0;
			return;
		}
	...
	obliczStratyPoWartosciStrat(value = this.input.stratyWartosc): void {
		...
		if (!isNumbers || (isNumbers && isLetters) || isNaN(value)) {
			this.input.stratyWartosc = new String('N');
			this.output.e = 0;
			return;
		}
	...
	obliczStratyPoLiczbieBudynkow(value = this.input.stratyLiczbaBud): void {
		...
		if (!isNumbers || (isNumbers && isLetters) || isNaN(value)) {
			this.input.stratyLiczbaBud = new String('N');
			this.output.e = 0;
			return;
		}
